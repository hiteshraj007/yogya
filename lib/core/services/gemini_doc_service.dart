...
class GeminiDocService {
  GeminiDocService._();
  static final GeminiDocService instance = GeminiDocService._();
  ...
  Future<OcrResult?> analyzeDocument(Uint8List imageBytes, {String ocrRawText = ''}) async {
    if (AiConfig.geminiApiKey.isEmpty) return null;

    try {
      final base64Image = base64Encode(imageBytes);
      final url = 'https://generativelanguage.googleapis.com/v1beta/models/${AiConfig.geminiModel}:generateContent?key=${AiConfig.geminiApiKey}';
      ...
      if (response.statusCode == 200) {
        String content = response.data['candidates'][0]['content']['parts'][0]['text'];
