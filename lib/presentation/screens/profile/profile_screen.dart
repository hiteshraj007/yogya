...
class _ProfileScreenState extends ConsumerState<ProfileScreen>
  ...
  void _loadSavedProfile() {
    ...
    if (profile != null) {
      ...
      setState(() {
        _selectedCategory = profile.category.isNotEmpty ? profile.category : 'General'; // 🔥 FIX
        _selectedGender = profile.gender.isNotEmpty ? profile.gender : 'Male';
        _gradStatus =
            profile.graduationStatus.isNotEmpty ? profile.graduationStatus : 'Pursuing';
      });
    } else if (user != null) {
      _nameCtrl.text = user.displayName ?? '';
      _emailCtrl.text = user.email ?? '';
  ...
  String _computeHighestQual() {
    if (_gradStatus == 'Completed' && _gradCourseCtrl.text.trim().isNotEmpty) {
      return 'Graduation (${_gradCourseCtrl.text.trim()})';
    }
    if (_twelfthBoardCtrl.text.trim().isNotEmpty) return '12th Pass';
    if (_tenthBoardCtrl.text.trim().isNotEmpty) return '10th Pass';
    return 'Not Specified';
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              _animItem(
                1,
                _buildSection('Personal Information', Icons.person_outline_rounded, [
                  AppTextField(label: 'Full Name', controller: _nameCtrl, prefixIcon: Icons.person_rounded),
                  const SizedBox(height: 12),
                  AppTextField(label: 'Date of Birth', controller: _dobCtrl, hintText: 'DD/MM/YYYY', prefixIcon: Icons.calendar_today_rounded),
                  const SizedBox(height: 12),
                  _buildGenderSelector(),
                ]),
              ),
              ...
              _animItem(
                2,
                _buildSection('Category', Icons.category_rounded, [
                  CategorySelector(
                    categories: ExamData.userCategories,
                    selected: _selectedCategory,
              ...
              _animItem(
                3,
                _buildSection('Academic Information', Icons.school_outlined, [
                  _readOnlyBox('Highest Qualification', _computeHighestQual()),
                  const SizedBox(height: 12),

                  _readOnlyBox('10th Board', _tenthBoardCtrl.text.isEmpty ? 'Not set' : _tenthBoardCtrl.text),
                  _readOnlyBox('10th Year', _tenthYearCtrl.text.isEmpty ? 'Not set' : _tenthYearCtrl.text),
                  _readOnlyBox('10th Percentage', _tenthPercentCtrl.text.isEmpty ? 'Not set' : _tenthPercentCtrl.text),

                  const SizedBox(height: 12),
                  AppTextField(label: '12th Board', controller: _twelfthBoardCtrl, prefixIcon: Icons.account_balance_rounded),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: AppTextField(label: '12th Year', controller: _twelfthYearCtrl)),
                    const SizedBox(width: 12),
                    Expanded(child: AppTextField(label: '12th Percentage', controller: _twelfthPercentCtrl)),
                  ]),

                  const SizedBox(height: 12),
                  AppTextField(label: 'Graduation Course', controller: _gradCourseCtrl),
                  const SizedBox(height: 12),
                  AppTextField(label: 'Graduation University', controller: _gradUniCtrl),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: AppTextField(label: 'Graduation Year', controller: _gradYearCtrl)),
                    const SizedBox(width: 12),
                    Expanded(child: AppTextField(label: 'CGPA / Percentage', controller: _gradPercentCtrl)),
                  ]),
                  const SizedBox(height: 12),
              ...
              _animItem(
                4,
                _buildSection('Contact Information', Icons.phone_outlined, [
                  AppTextField(label: 'Email', controller: _emailCtrl, prefixIcon: Icons.email_outlined),
                  const SizedBox(height: 12),
                  AppTextField(label: 'Phone', controller: _phoneCtrl, prefixIcon: Icons.phone_rounded),
                ]),
              ),
              ...
              _animItem(
                5,
                _buildSection('Additional Information', Icons.info_outline_rounded, [
                  AppTextField(label: 'State of Domicile', controller: _stateCtrl, prefixIcon: Icons.location_on_rounded),
                  const SizedBox(height: 12),
                  AppTextField(label: 'Primary Exam Goal', controller: _examGoalCtrl, prefixIcon: Icons.flag_rounded),
                ]),
              ),
        ...
        children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 8),
              LinearProgressIndicator(value: completion / 100, minHeight: 5),
            ]),
          )
        ],
      ),
        ...
        return Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(10),
                border: Border.all(color: selected ? context.colors.primary : context.colors.glassBorder),
                color: selected ? context.colors.primary.withOpacity(0.15) : context.colors.bgCardLight,
              ),
  ...
  Widget _readOnlyBox(String label, String value) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: Text(label, style: TextStyle(color: context.colors.textSecondary, fontSize: 12))),
